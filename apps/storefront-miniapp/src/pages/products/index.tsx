...
import { Button, Image, Input, RootPortal, ScrollView, Text, View } from '@tarojs/components';
...
// Phase 2.52E：干货分类导航改为前端 deterministic 映射（见 lib/product-category-nav）。
const PRODUCT_CATEGORIES = DRY_NAV_CATEGORIES;
// Phase 2.52E-3：迟滞阈值（避免边界闪烁）。向下滚过 SHOW 显示；回到 HIDE 以内才隐藏。
const STICKY_SHOW_SCROLLTOP = 500;
const STICKY_HIDE_SCROLLTOP = 160;
...
export default function ProductsPage() {
  ...
  async function loadProducts() {
    ...
    try {
      if (channel === 'fresh') {
        ...
        // Phase 2.52E：一次拉取全部干货（不再按 category 走后端），分类改为前端 deterministic 过滤。
        // 搜索关键词仍走后端；分类在前端 useMemo 过滤，切换分类无需重新请求。
  ...
  useEffect(() => {
    ...
    loadProducts();
    // Phase 2.52E：activeCategory 不再触发后端请求（前端过滤），故从依赖移除。
  }, [searchKeyword, channel]);

  // Phase 2.52E：按当前分类前端过滤（全部/今日推荐/11 个真实分类）。
  const displayedProducts = useMemo(() => {
    if (activeCategory === '全部') return products;
    if (activeCategory === '今日推荐') return products.filter((p) => p.isRecommended === true);
    return products.filter((p) => classifyDryProduct(p.name, p.category) === activeCategory);
  }, [products, activeCategory]);

  // Phase 2.52E-3：以「页面级 scrollTop」为唯一数据源 + 迟滞开关，控制吸顶条/回顶部显隐。
  // 之前 bug：FAB/吸顶条挂在 .page-fade-in（animation fill:both 残留 transform）内，
  // transform 祖先使 position:fixed 锚到该容器而非视口 → 随页滚动、只在顶部附近闪现。
  // 修复：显隐用单一 page scrollTop + 迟滞；挂载改用 RootPortal 逃出 transform 包含块（见下方 JSX）。
  const [stickyVisible, setStickyVisible] = useState(false);
  usePageScroll((res) => {
    const y = res.scrollTop;
    setStickyVisible((prev) => {
      if (y >= STICKY_SHOW_SCROLLTOP) return true;
      if (y <= STICKY_HIDE_SCROLLTOP) return false;
      return prev; // 迟滞区间：维持当前状态，避免抖动
    });
  });
  // 切换频道时重置（避免刚切频道就莫名出现，需重新滚过阈值才显示）。
  useEffect(() => {
    setStickyVisible(false);
  }, [channel]);
  // 打开任一弹窗时立即收起。
  useEffect(() => {
    if (skuSheetProduct || freshSheetItem) setStickyVisible(false);
  }, [skuSheetProduct, freshSheetItem]);

  // Phase 2.52E-4：回顶部 FAB 扩展到 dry + fresh；吸顶分类条仍仅 dry（fresh 有自己的分类区，不复用干货分类）。
  const noSheetOpen = !skuSheetProduct && !freshSheetItem;
  const showBackToTop = (channel === 'dry' || channel === 'fresh') && stickyVisible && noSheetOpen;
  const showStickyCatBar = channel === 'dry' && stickyVisible && noSheetOpen;

  function scrollBackToFilter() {
    // 按当前频道选择顶部锚点：dry→干货搜索/分类区；fresh→鲜鱼频道顶部（双频道入口+鲜鱼分类）。
    const anchor = channel === 'fresh' ? '#fresh-filter-anchor' : '#dry-filter-anchor';
    Taro.pageScrollTo({ selector: anchor, duration: 300 }).catch?.(() => {
  ...
  return (
    <View className='page-fade-in' style={{ minHeight: '100vh', padding: '28rpx', background: channel === 'fresh' ? '#f1faf8' : '#fff8ea' }}>
      ...
      {/* Phase 2.52E-4：作为 fresh 频道「回顶部」锚点（其下即频道切换 + 鲜鱼分类/列表）。 */}
      <View id='fresh-filter-anchor' style={{ display: 'flex', gap: '16rpx', marginBottom: '24rpx' }}>
        {([
      ...
      <View className='commerce-filter-card' id='dry-filter-anchor'>
      ...
      {loading ? (
        ...
        <View className='brand-empty-card'>
          ...
          <Text style={{ display: 'block', fontSize: '32rpx', fontWeight: '700', color: '#17231c', marginBottom: '10rpx' }}>
            {products.length === 0 ? '暂无可售商品' : `「${activeCategory}」暂无商品`}
          </Text>
          <Text style={{ display: 'block', fontSize: '26rpx', color: '#6b7280', marginBottom: '22rpx' }}>
            {products.length === 0 ? '暂时没有可售商品，请稍后再来或联系商家。' : '试试切换其它分类或使用搜索。'}
          </Text>
          {products.length > 0 ? (
            <Button className='ui-pressable commerce-clear-button' hoverClass='ui-pressable-hover' size='mini' onClick={() => setActiveCategory('全部')}>
              查看全部
            </Button>
          ) : null}
        </View>
        ...
        <View style={{ display: 'flex', flexDirection: 'column', gap: '22rpx' }}>
          <View style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <Text style={{ fontSize: '26rpx', color: '#6b7280' }}>
              当前可选 {displayedProducts.length} 款{activeCategory !== '全部' ? ` · 共 ${products.length} 款` : ''}
            </Text>
      ...
      {/* Phase 2.52E-3：吸顶分类条 + 回顶部按钮统一放进 RootPortal，挂到页面根，
          逃出 .page-fade-in 的 transform 包含块，确保 position:fixed 真正相对视口固定。 */}
      {showBackToTop ? (
        <RootPortal>
          {/* 吸顶分类条：仅干货（fresh 有独立分类区，不复用干货分类）。文字 chip，不放图标避免拥挤。 */}
          {showStickyCatBar ? (
            <View className='sticky-cat-bar'>
              <ScrollView scrollX className='sticky-cat-scroll' showScrollbar={false}>
                <View className='sticky-cat-inner'>
                  {PRODUCT_CATEGORIES.map((category) => (
                    <View
                      key={category}
                      className={`sticky-cat-chip ${activeCategory === category ? 'sticky-cat-chip-active' : ''}`}
                      onClick={() => setActiveCategory(category)}
                    >
                      <Text>{category}</Text>
                    </View>
          ...
          {/* 回顶部悬浮按钮：干货 + 鲜鱼共用（右侧中部偏下，避开 tabbar，按频道回对应顶部锚点） */}
          <View className='back-to-top-fab' onClick={scrollBackToFilter} hoverClass='ui-pressable-hover'>
            <Text className='back-to-top-arrow'>↑</Text>
